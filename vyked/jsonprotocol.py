...
class JSONProtocol(asyncio.Protocol):
    ...
    def _make_frame(packet):
        ...
        if isinstance(packet, dict) and 'type' in packet and packet['type'] in ['request', 'response']:
                ...
                for e in split_array:
                    if e:
                        try:
                            ...
                            if json_loads_time >= _VALID_MAXIMUM_JSON_LOADS_TIME_IN_MS and isinstance(element, dict) and \
                                'type' in element and element['type'] in ['request', 'response']:
